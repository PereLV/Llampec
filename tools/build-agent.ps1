        ...
        switch ($cmd) {
            ...
            'commit' {
                if (-not (Test-Path $commitMsg)) {
                    ...
                    # .claude-build/ is already in .gitignore, so a plain `git add -A` already skips it --
                    # no pathspec exclusion needed. (The previous version wrapped one in single quotes, which
                    # Invoke-Proc passes straight to Win32 process creation with no shell in between to strip
                    # them, so git saw the literal characters ':!.claude-build' quotes included, and failed.)
