...
/// <summary>
/// A hidden message-only window that receives system broadcasts (display changes, theme changes,
/// hotkeys, tray icon callbacks) and exposes them as events. It is the only HWND that exists while the
/// panel is closed, so the app costs nothing while idle: no timers, no polling.
