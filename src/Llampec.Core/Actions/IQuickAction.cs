...
public interface IQuickAction
{
    ...
    /// <summary>
    /// True when <see cref="SubActions"/> are mutually exclusive -- exactly one is ever On, like the
    /// Projection modes (PC screen only / Duplicate / Extend / Second screen only). The panel renders such
    /// a sub-page as a radio-button list instead of independent toggle switches (e.g. HDR's per-monitor
    /// switches, where several can legitimately be On at once).
    /// </summary>
    bool SubActionsAreExclusive { get; }
