...
public sealed class TileViewModel : ObservableObject
{
    ...
    public TileViewModel(IQuickAction action, Dispatcher dispatcher, Action<TileViewModel>? openSubpage)
    {
        Action = action;
        _dispatcher = dispatcher;
        _openSubpage = openSubpage;

        ToggleCommand = new RelayCommand(() => _ = Action.ExecuteAsync(CancellationToken.None));
        OpenSubpageCommand = new RelayCommand(() => _openSubpage?.Invoke(this), () => HasSubpage);

        SubTiles = action.SubActions.Select(a => new TileViewModel(a, dispatcher, null)).ToList();
